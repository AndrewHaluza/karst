...
- Empirical, against the SHIPPED file: `docs/design/stages/harness.mjs` renders `src/ui/dashboard/webview.html` itself against eight fake `DashboardState` snapshots and exposes a width sweep. That is what the floor is measured on — the mockup approximates the tokens and renames three of the classes, so its number is the design's, not the product's. Measured: no overflow to **280px**, the heaviest case scrolling (never clipping) at 260px, and the current segment keeping its name and action button to 220px. `docs/design/stages/variant-e-track.html` stays the design record.

The harness also caught the one class of defect a text pin cannot: scoping the base rule to `.track .seg` (specificity 0,2,0) beat the injected `stg-<stage>` palette classes (0,1,0) at any source order, so every travelled segment rendered neutral grey and the stage-hue wash was silently dead. The base rule must therefore set no `color` and no `--stg-color`; `webview.test.ts` pins that, and the status classes override the hue deliberately at (0,3,0).
