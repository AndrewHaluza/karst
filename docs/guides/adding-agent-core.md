...
- There is no CLI hook flag; lifecycle bridging is a generated plugin (Task 7): the adapter writes `.opencode/plugins/karst-bridge.js` beneath the worktree and launches WITHOUT `--pure` — opencode's `--pure` disables ALL external plugin loading, including the auto-discovered karst-bridge, so an interactive session launched with it can never deliver a hook event (869eg458d). The plugin POSTs `session.created` (normalized to `SessionStart`), `session.updated` (token usage — see below), `session.idle`/`session.error`/`permission.asked` (and question asks, normalized to `permission.asked`) to the loopback endpoint; the adapter advertises `lifecycleEvents: true` only because the channel ships with it. **`session.created` → `SessionStart` is load-bearing for the launch-intent handshake**: a closed-session fix launch records its intent and awaits a `SessionStart` with the launch id to confirm it (`confirmFixLaunch`) — without that mapping the round stays `pending`, never `fixing`, and the stranded-fix sweep parks the stage "no fix execution in flight" while the agent is actually working (REVIEW-2ND-ROUND-FIX-STUCK-WITH). Headless `opencode run` still passes `--pure` for gate isolation.
- **The plugin must bridge the WAIT RESOLUTION, not just the wait.** opencode has no `PostToolUse`/`UserPromptSubmit`, so nothing ever flips a `permission.asked` amber back to `running` — one answered prompt left the ticket reading "Needs you" for the whole remaining turn (FIX-WRONG-STATUS). The plugin therefore also POSTs the ask resolutions (`permission.replied`/`permission.v2.replied`/`question.replied`/`question.v2.replied`, normalized to `permission.replied`) and `session.status` busy/retry (with the status type as `message`) — both map to `running` in `nextAgentState`. A new provider with a push channel that lacks a processing signal needs the equivalent.
