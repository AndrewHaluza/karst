...
New `Manifest` field checklist: add to `types.ts` + `validateManifest` (schema.ts, default it) + **`writeManifest` overlay (write.ts)** or Save silently drops it. Guard: writeManifest.test.ts "round-trips every modeled section". Repository/service fields also go in `manifest/fixtures.ts` — the shared test builders every suite uses, so a shape change is one file, not 26. `agentPresets`/`defaultAgentPreset` follow the checklist: typed in `types.ts`, validated in `validate/agentPresets.ts` (plus reference integrity against `processes.<key>.preset`), and overlaid in `write.ts`. `agentPresets` is claimed by no Settings section, so it always survives from the base on a tab-scoped Save; `defaultAgentPreset` is a General-tab field and the process rows carry the per-role `preset`.