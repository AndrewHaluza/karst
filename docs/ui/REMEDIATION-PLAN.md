...
| ☑ 2.4 | Close the `busy` vocabulary in `onboarding/messages.ts` (`what: string` → union) **and fix the swallowed `'suggest'` case** — a live defect: the host posts it, the webview's switch has no branch, so Suggest never shows pending. | R16 |
...
| ☑ 3.5 | `onboarding` | 1361 | ~30 | `.ghost` class with no rule; `#detailsBtn` no `aria-expanded`; `.chip`/`.acard` non-native with no `aria-pressed`/`aria-checked`; stepper has no `aria-current`; `#sig-*` inputs unlabelled; attach/paste/detach have no pending UI during file I/O; one shared `#err` for every failure |
