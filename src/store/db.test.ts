...
describe('openStore', () => {
  ...
  it('creates all 17 registry tables', () => {
  ...
  it('reports the current schema user_version', () => {
    const store = openStore(':memory:');
    cleanups.push(() => store.close());
    expect(store.db.pragma('user_version', { simple: true })).toBe(26);
  });
  ...
  it('migrates a v16 DB to v17 with attachment lookup and dedupe indexes without touching rows', () => {
    ...
    expect(migrated.db.pragma('user_version', { simple: true })).toBe(26);
  });
  ...
  it('is a no-op on a fresh DB, which already has repo columns', () => {
    ...
    expect(reopened.db.pragma('user_version', { simple: true })).toBe(26);
  });
