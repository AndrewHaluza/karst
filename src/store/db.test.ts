...
describe('openStore', () => {
  ...
  it('reports the current schema user_version', () => {
    const store = openStore(':memory:');
    cleanups.push(() => store.close());
    expect(store.db.pragma('user_version', { simple: true })).toBe(38);
  });
  ...
  it('migrates a database at v34 straight to v36 (graph tables + blocked_reason)', () => {
    ...
    expect(migrated.db.pragma('user_version', { simple: true })).toBe(38);
  });
  ...
  it('an interrupted v35 step leaves user_version at 34 and the next open completes', () => {
    ...
    expect(reopened.db.pragma('user_version', { simple: true })).toBe(38);
  ...
  it('two live opens of one file migrate without corruption', () => {
    ...
    expect(a.db.pragma('user_version', { simple: true })).toBe(38);
    expect(b.db.pragma('user_version', { simple: true })).toBe(38);
