...
export class OpencodeAdapter implements AgentAdapter {
  ...
  async runHeadless(opts: RunHeadlessOpts): Promise<HeadlessResult> {
    ...
    // `buildInteractiveCommand` gives the interactive session: a headless run
    // (e.g. a PR description) must not inherit plugins that add context,
    // latency, or other projects' hook channels (869ef1e6x).
