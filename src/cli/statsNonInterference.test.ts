...
/**
 * Static non-interference guard for `karst stats`.
 *
 * The report is a READ. It must never mutate a stage, a gate, a session or the
 * manifest, and must never spawn a process or open a socket — not even
 * transitively, because a single convenience import is enough to pull a writer
 * into the graph. So the reachable module set is what this asserts, not the
 * entry files alone. Same shape as `diagnostics/nonInterference.test.ts`.
 */

const ENTRY_POINTS = ['src/cli/stats.ts', 'src/store/metrics/index.ts'] as const;
...
describe('karst stats non-interference', () => {
  ...
  it('reaches the metric modules it is supposed to', () => {
    // A guard over an empty graph proves nothing — pin that the walk found them.
    expect(graph.files.has('src/store/metrics/delivery.ts')).toBe(true);
    expect(graph.files.has('src/store/metrics/cost.ts')).toBe(true);
  });

  it('cannot reach process, network, or state-mutating modules', () => {
  ...
  it('issues no write SQL anywhere in the reachable graph', () => {
    ...
    for (const file of graph.files) {
      ...
      for (const line of source.split('\n')) {
        if (
          /\b(INSERT\s+INTO|UPDATE\s+\w+\s+SET|DELETE\s+FROM|DROP\s+|ALTER\s+TABLE|CREATE\s+TABLE)/i
            .test(line)
        ) {
