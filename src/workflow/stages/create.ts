...
export function createTicketFlow(store: Store, input: CreateTicketInput): Ticket {
  // Idempotent by key: recreating (or refetching) the same ticket reuses the
  // existing row instead of duplicating it. Existing fields are left untouched;
  // callers refresh onboarding fields separately.
  const existing = getTicketByKey(store, input.key);
  if (existing) {
    ...
    if (existing.archivedAt !== null) {
      ...
      return getTicketByKey(store, input.key)!; // refreshed (archivedAt cleared)
    }
    return existing;
  }
