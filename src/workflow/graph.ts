 ...
 *                            └─ fix ←──────────┘
 *
 * `ship` ends where the PRs exist, NOT where the work landed. There is no
 * standalone stage for the gap between the two anymore — a `merge` node used to
 * own it, but a ticket could still read Done while every one of its PRs sat
 * unmerged, because "done" and "landed" were two different claims sharing one
 * word. The gap is now an ENTRY GATE on `done` itself
 * (`workflow/mergeGate.ts`'s `resolveShipLanding`/`settleShipGate`): ship's own
 * `passed` verdict only completes the transition to `done` once every PR it
 * opened reads merged upstream (or the ticket delivered nothing to merge at
 * all); until then the ticket stays parked at `ship`, blocked
 * (`awaiting-merge`, `model/types.ts`) rather than advancing.
 *
 ...
 *    dashboard's "Confirm ship" click. Landing them is more irreversible still,
 *    and stays a human's click too (the per-repo "Merge" button, or a teammate
 *    merging upstream, which the PR sweep notices) — but that no longer has a
 *    stage of its own to park at; `ship` itself carries the wait, via the
 *    `awaiting-merge` block described in `mergeGate.ts`.
 *
